...
export const fetchUsers = () => async dispatch => {
    try {
        const response = await axiosConfig.get('/bankingcore/listofaccounts');
...
export const approveDeposit = (transactionId) => async dispatch => {
    try {
        await axiosConfig.post(`/bankingcore/deposits/approve/${transactionId}`);
...
export const rejectDeposit = (transactionId) => async dispatch => {
    try {
        await axiosConfig.post(`/bankingcore/deposits/reject/${transactionId}`);
...
export const fetchPendingDeposits = () => async dispatch => {
    try {
        const response = await axiosConfig.get('/bankingcore/deposits/getPendingDeposits');
        if (response.data) {
            dispatch({
...
export const fetchAllDocuments = async () => {
    try {
        const response = await axiosConfig.get("/Files/getAllFiles");
...
export const downloadDocument = async (documentId) => {
    try {
        const response = await axiosConfig.get(`/Files/downloadFile/${documentId}`, {
            responseType: 'blob'
        });
        return response;
    } catch (error) {
